...
class ListaComasParser ( Parser ):
    ...
    symbolicNames = [ "<INVALID>", "COMA", "NUMERO", "WS" ]
    ...
    class ListaContext(ParserRuleContext):
        ...
        def COMA(self, i:int=None):
            if i is None:
                return self.getTokens(ListaComasParser.COMA)
            else:
                return self.getToken(ListaComasParser.COMA, i)
    ...
    def lista(self):
        ...
        try:
            ...
            while _la==1:
                self.state = 3
                self.match(ListaComasParser.COMA)
                self.state = 4
                self.match(ListaComasParser.NUMERO)
                self.state = 9
                self._errHandler.sync(self)
